...
class InterwettenWorkflow(ProviderWorkflow):
    ...
    async def _find_outcome_element(self, page: Page, bet) -> dict | None:
        ...
        spread_point = _g(bet, "spread_point")  # e.g. -1.5 for home spread
            ...
            result = await page.evaluate(
                ...
                (args) => {
                    ...
                    for (const grid of grids) {
                        ...
                        // For spread: parse handicap from market_type like "Handicap H:A" or "Handicap -1.5"
                        let gridSpreadPoint = null;
                        if (market === 'spread' || market === 'total') {
                            const hcapMatch = marketType.match(/([+-]?[\\d.]+)/);
                            if (hcapMatch) gridSpreadPoint = parseFloat(hcapMatch[1]);
                        }
