...
class LimitRepo:
    ...
    def get_by_id(self, limit_id: int) -> ProfileProviderLimit | None:
        return self.db.query(ProfileProviderLimit).filter(ProfileProviderLimit.id == limit_id).first()
    ...
    def get_banned_providers(self, profile_id: int) -> set[str]:
        ...
        rows = (
            self.db.query(ProfileProviderLimit.provider_id)
            .filter(
    ...
    def get_existing(self, profile_id: int, provider_id: str, limit_type: str) -> ProfileProviderLimit | None:
        return (
