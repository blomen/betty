...
if __name__ == "__main__":
    print("=== Free Bet Example ===")
    print("$100 free bet at Unibet, odds 3.00")
    print("Hedge at Bet365, odds 1.50 (opposing outcome)")
    print()
    
    result = calculate_free_bet_value(
        free_bet_amount=100,
        anchor_odds=3.00,
        hedge_odds=1.50,
        stake_returned=False,  # SNR free bet
    )
    
    print(f"Free bet wins: ${100 * (3.00 - 1):.2f} profit")
    print(f"Hedge stake needed: ${result['hedge_stake']:.2f}")
    print(f"Guaranteed profit: ${result['profit']:.2f}")
    print(f"Retention: {result['retention_pct']:.1f}% of free bet value")
    
    print()
    print("=== Qualifying Bet Example ===")
    print("$50 qualifying bet at Unibet, odds 2.00")
    print("Find best hedge...")
    print()
    
    hedges = [
        {"provider": "bet365", "outcome": "away", "odds": 2.10},
        {"provider": "betsson", "outcome": "away", "odds": 2.05},
        {"provider": "unibet", "outcome": "away", "odds": 2.00},  # Same provider, skip
    ]
    
    match = find_best_hedge(
        event_id="test",
        market="1x2",
        ...
        anchor_odds=2.00,
        anchor_stake=50,
        opposing_odds_list=hedges,
        is_free_bet=False,
    )
    
    if match:
        print(f"Best hedge: {match.hedge_provider} @ {match.hedge_odds}")
        print(f"Hedge stake: ${match.hedge_stake:.2f}")
        print(f"Qualifying loss: ${match.qualifying_loss:.2f}")
