...
def get_extraction_settings(db: Session = Depends(get_db)):
    ...
    extraction_tiers = config.get("extraction_scheduling", {})
    providers_config = config.get("providers", {})

    # Load DB overrides for active profile
    overrides = {
        s.provider_id: s.enabled
        for s in db.query(ProviderExtractionSetting).filter(ProviderExtractionSetting.profile_id == profile_id).all()
    }
...
def toggle_extraction_provider(
    ...
    existing = db.query(ProviderExtractionSetting).get((profile_id, body.provider_id))
