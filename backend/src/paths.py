...
Uses environment variables with defaults:
  BETTY_DATA_DIR    → /app/data (Docker) or backend/data (dev)
  BETTY_LOGS_DIR    → /app/logs (Docker) or backend/logs (dev)
  BETTY_CONFIG_DIR  → src/config (always relative to source)
"""
...
def get_data_dir() -> Path:
    ...
    d = Path(os.environ.get("BETTY_DATA_DIR", str(_BACKEND_DIR / "data")))
...
def get_logs_dir() -> Path:
    """Logs directory."""
    d = Path(os.environ.get("BETTY_LOGS_DIR", str(_BACKEND_DIR / "logs")))
    d.mkdir(parents=True, exist_ok=True)
    return d
...
def get_config_path(filename: str) -> Path:
    ...
    config_dir = Path(os.environ.get("BETTY_CONFIG_DIR", str(Path(__file__).parent / "config")))
    return config_dir / filename


def get_config_dir() -> Path:
    """Config directory."""
    return Path(os.environ.get("BETTY_CONFIG_DIR", str(Path(__file__).parent / "config")))
