"""
Centralized path resolution for BankrollBBQ.
...
In bundled mode:
  ...
  - User data → %LOCALAPPDATA%/BankrollBBQ/
"""
...
def get_app_data_dir() -> Path:
    ...
    Bundled: %LOCALAPPDATA%/BankrollBBQ/
    Dev:     backend/ (preserves current behavior)
    """
    if is_bundled():
        local = os.environ.get('LOCALAPPDATA', str(Path.home() / 'AppData' / 'Local'))
        app_dir = Path(local) / 'BankrollBBQ'
...
def get_config_path(filename: str) -> Path:
    ...
    Checks %LOCALAPPDATA%/BankrollBBQ/config/ first (user customization),
...
def get_env_path() -> Path:
    ...
    Bundled: %LOCALAPPDATA%/BankrollBBQ/.env
    Dev:     backend/.env
    """
