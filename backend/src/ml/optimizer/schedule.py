...
FEATURE_NAMES = [
    ...
    "day_of_week",
    "tier_encoded",
    "minutes_since_last_sharp",
    "minutes_since_last_soft",
    "events_starting_next_2h",
    "events_starting_next_6h",
    "providers_attempted",
    "providers_succeeded",
    "providers_failed",
...
class ScheduleOptimizer:
    ...
    def check_and_train(self, session) -> dict | None:
        ...
        result = train_model(
            X, y, task="regression", min_samples=self.activation_threshold, feature_names=FEATURE_NAMES
        )
        if result is None:
            return None
    ...
    def _enrich_with_history(self, rows: list) -> list[dict]:
        ...
        for row in rows:
            ...
            hist = {
                ...
                "value_bets_avg_last_5": (sum(past_yields[-5:]) / len(past_yields[-5:]) if past_yields else 0),
