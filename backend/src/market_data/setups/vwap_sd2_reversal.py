...
def detect_vwap_sd2_reversal(ctx: DetectorContext) -> list[SetupCandidate]:
    ...
    if at_upper and price > vwap.vwap:
        ...
        exhaustion = of.cvd_trend in ("flat", "falling") or of.passive_active_ratio > 1.5 or of.delta_pct < -0.2
        if exhaustion:
    ...
    if at_lower and price < vwap.vwap:
        exhaustion = of.cvd_trend in ("flat", "rising") or of.passive_active_ratio > 1.5 or of.delta_pct > 0.2
        if exhaustion:
