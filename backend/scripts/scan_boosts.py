...
SITES = {
    ...
    'interwetten': 'https://www.interwetten.se',
    # Kambi
...
BOOST_KW = [
    'odds boost', 'oddsboost', 'odds-boost',
    'förhöjda odds', 'förhöjda', 'guldboost',
    'super boost', 'superboost', 'ökade odds',
    'enhanced', 'boosted', 'priceboost', 'dagens spel',
]

API_KW = ['boost', 'bonus', 'featured', 'special', 'enhanced',
           'highlight', 'promo', 'priceboost', 'globalbonuses']


async def scan_site(context, name, url):
    """Scan a single site for boost content."""
    result = {'name': name, 'url': url, 'boost_apis': [], 'ws_boost': [],
              'keywords_html': [], 'keywords_visible': [], 'all_api_count': 0, 'error': None}
    ...
    try:
        ...
        # Cookie consent
        for sel in ['#onetrust-accept-btn-handler', 'button:has-text("Acceptera")',
                     'button:has-text("Accept")', 'button:has-text("Godkänn")']:
            try:
