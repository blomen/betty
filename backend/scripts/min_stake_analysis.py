"""
BankrollBBQ — Min Stake Strategy Analysis
==========================================
...
def kelly_fraction(edge_pct, min_kelly=0.25, max_kelly=0.75):
    if edge_pct <= 2.0: return min_kelly
    elif edge_pct >= 6.0: return max_kelly
    t = (edge_pct - 2.0) / 4.0
    return min_kelly + t * (max_kelly - min_kelly)


def round_stake_natural(stake):
    if stake < 50: return max(5.0, round(stake / 5) * 5)
    elif stake < 200: return round(stake / 10) * 10
    elif stake < 500: return round(stake / 25) * 25
    else: return round(stake / 50) * 50
    ...
    for _ in range(50):
        ...
        for min_e, max_e, weight, avg_odds in edge_dist:
            ...
            if r <= cum:
                ...
                if odds >= min_odds: return edge, odds
                break
...
def stake_strategy_skip(bankroll, edge_pct, odds, cap_pct=0.03, min_stake=25.0, mep=2.0):
    ...
    if stake < min_stake: return 0.0
    if mep > 0 and stake * edge < mep: return 0.0
    return stake
...
def stake_strategy_bump(bankroll, edge_pct, odds, cap_pct=0.03, min_stake=25.0, mep=0.75):
    ...
    if stake < ABSOLUTE_MIN_STAKE: return 0.0
    if mep > 0 and stake * edge < mep: return 0.0
    return stake
...
def simulate(starting, bpw, weeks, strategy_fn, track_weekly=False):
    ...
    for w in range(weeks):
        ...
        for _ in range(n):
            if bankroll < ABSOLUTE_MIN_STAKE:
                return SimResult(bankroll, profit, played, skipped, staked,
                                peak, trough, max_dd, True, weekly)
    ...
    return SimResult(bankroll, profit, played, skipped, staked,
                    peak, trough, max_dd, bankroll < ABSOLUTE_MIN_STAKE, weekly)
...
def coverage_test():
    ...
    for name, fn, desc in STRATEGIES:
        print(f"\n  {name}  ({desc})")
        print(f"  {'Bankroll':>10s}  {'Playable':>9s}  {'Skipped':>8s}  {'Avg Stake':>10s}  {'Avg EV':>8s}  {'Stake/BR':>9s}")
...
def growth_comparison():
    ...
    for br in bankrolls:
        ...
        print(f"  {'━'*96}")
        print(f"  {'Strategy':<24s}  {'Med Final':>10s}  {'Growth':>7s}  {'P10':>10s}  "
        ...
        for name, fn, desc in STRATEGIES:
            ...
            print(f"  {name:<24s}  {mf:>10,.0f}  {gr:>+6.0f}%  {p10:>10,.0f}  "
...
def bump_analysis():
    ...
    for br in bankrolls:
        ...
        print(f"    Normal (≥25 kr):   {normal:>6,}  ({normal/total*100:.1f}%)  avg EV: {normal_ev/max(1,normal):.2f} kr")
        print(f"    Bumped (5→25 kr):  {bumped:>6,}  ({bumped/total*100:.1f}%)  avg EV: {bumped_ev/max(1,bumped):.2f} kr  avg overbet: {bumped_overbet/max(1,bumped):.3f}% of BR")
        ...
        if bumped > 0:
            ...
            print(f"    Per-scan equivalent (~2039 opps): ~{bumped/n_bets*2039:.0f} bumped bets, ~{bumped_ev/n_bets*2039:.0f} kr extra EV")
...
def main():
    ...
    print("=" * 100)
    print("  BANKROLLBBQ — MIN STAKE STRATEGY ANALYSIS")
