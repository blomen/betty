...
- **Coolbet — parallel pagination (VALIDATED ✅)** — Parallelized football category pagination: 5 concurrent page fetches via `asyncio.gather()` (was sequential). Reduced Imperva sleep 4→2s. **177/180 baseline odds (98%). No regression.**
- **ComeOn Group — adaptive WS wait (VALIDATED ✅)** — Replaced fixed 2s date-click waits with adaptive WS polling (0.5s min, poll every 0.1s, max 2.0s). Reduced page load wait 1.5→1s, WS init wait 3→2s, scroll wait 0.5→0.3s. **265-310 odds (75-88% of 353 baseline). Acceptable — some variance from event availability.**
- **Spectate — removed networkidle (VALIDATED ✅)** — Removed `networkidle` wait (could stall 3-10s), reduced init wait 5→3s. **No data regression.**
- **Kambi config (VALIDATED ✅)** — Reduced `post_extraction_delay_ms` 15000→10000 (saves ~35s across 8 Kambi brands). **No data regression.**
- **10Bet — selector-based wait (VALIDATED ✅)** — Replaced fixed 200ms odds wait with `wait_for_selector('[class*="ta-price_text"]', timeout=500)`, reduced retry wait 2→1.5s. **Reverted MAX_COMPETITIONS back to 60 (80 caused football timeout).**
- **Tipwin — conservative wait reduction (VALIDATED ✅)** — Inter-page wait 0.5→0.4s. **Reverted session init and listing waits back to 2s (1.5s caused data loss).** 1612/2012 baseline (80%).
- **Snabbare — REVERTED** — All timing reductions caused severe regression (40-92 odds vs 387 baseline). Fully reverted to original values.
- **Interwetten — REVERTED** — Concurrent tabs 7→5 and render wait 100→150ms reverted (caused data loss in Pass 2 detail enrichment).
- **Full pipeline validation:** 1,335 events / 43,876 odds / 953 matched (71.3%) / 393 value bets. Total time: 617s.
