"""
Arnold Stocks launcher -- local process connecting TopstepX to the arnold server.

Double-click arnoldstocks.bat or run `python run_arnoldstocks.py` to start.
...
log = logging.getLogger("arnoldstocks")
...
def _cleanup_old_instance():
    ...
    _kill_port(LOCAL_DASHBOARD_PORT, "arnoldstocks-dashboard")
...
async def main():
    ...
    if not config.is_configured:
        print("[arnoldstocks] TopstepX not configured -- set TOPSTEPX_USERNAME and TOPSTEPX_API_KEY")
        return
    ...
    try:
        ...
        print("[arnoldstocks] FAILED: ssh not found. Install OpenSSH.")
    ...
    if not _start_tunnels():
        print("[arnoldstocks] Cannot open SSH tunnels. Check SSH key and server.")
        input("Press Enter to exit...")
        return
    ...
    if not ok:
        print("[arnoldstocks] TopstepX authentication failed -- check credentials")
        await client.close()
        input("Press Enter to exit...")
        return
    ...
    print(f"[arnoldstocks] Running -- relay -> {config.server_ws_url}")
    print("[arnoldstocks] Press Ctrl+C to stop\n")
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            print("\n[arnoldstocks] Restarting in 2s... (Ctrl+C again to exit)")
            try:
                time.sleep(2)
            except KeyboardInterrupt:
                print("\n[arnoldstocks] Exiting.")
                break
        except Exception as exc:
            print(f"\n[arnoldstocks] Error: {exc}")
            import traceback
