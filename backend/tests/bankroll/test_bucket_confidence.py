...
class TestCalculateStakeWithMultiplier:
    """End-to-end: confidence_multiplier flows through calculate_stake."""

    def test_multiplier_one_is_noop(self):
        from src.bankroll.stake_calculator import calculate_stake

        baseline = calculate_stake(
        ...
        assert baseline.stake == with_mult.stake

    def test_multiplier_half_halves_kelly_stake(self):
        from src.bankroll.stake_calculator import calculate_stake

        # Pick an edge/odds combo where the raw Kelly stake is below the
        # single-bet cap so a 0.5 multiplier on Kelly results in a ~0.5×
        # stake (rather than being clipped by the cap or min_stake floor).
        base = calculate_stake(
            bankroll_total=10000,
            edge_raw=0.03,
            odds=2.0,
            min_odds=0.0,
        ...
        assert base.stake > 0
        assert half.stake == pytest.approx(base.stake * 0.5, rel=0.01)

    def test_multiplier_zero_skips_bet(self):
        from src.bankroll.stake_calculator import calculate_stake
        ...
        assert "bucket confidence" in (result.skip_reason or "").lower()
