...
async def sync(
    ...
    for pos in positions:
        ...
        payload = {
            ...
            # Tagging as "mirror" matches the soft-book reactive recovery path —
            # it bypasses the edge gate (the user already accepted the price)
            # and stays out of arb-correlation analytics that look at arb_counter.
            # If this row truly is an arb counter, the arb_runner side will have
            # ALREADY inserted it with bet_type="arb_counter" + arb_group_id
            # before this recovery pass fires, and the (event_id, outcome) dedup
            # above will skip it (recovery only inserts unseen positions).
            "bet_type": "mirror",
        }
