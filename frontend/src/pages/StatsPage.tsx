...
import { BankrollChart, CLVChart, CHART } from '@/components/stats/charts';
...
export function BetsPage() {
  ...
  // Sub-tab: Bets (existing content) vs Shadow CLV (opp_snapshots)
  const [subTab, setSubTab] = usePersistedState<'bets' | 'shadow'>('bbq_stats_subTab', 'bets');
      ...
      <div className="flex items-center gap-1 -mx-1 -mt-1">
        <button
          ...
          className={`px-3 py-1 text-[11px] font-semibold uppercase tracking-wider rounded transition-colors ${
            subTab === 'bets'
              ? 'bg-tabBets/30 text-tabBets border border-tabBets/40'
        ...
        <button
          ...
          title="CLV for every scanner-detected opp (played + unplayed)"
        >
          Shadow CLV
        </button>
      </div>
      ...
      <div>
        ...
        {!analyticsCollapsed && activeAnalytics && (
          <div className="grid grid-cols-2 gap-2 mt-2">
            ...
            {activeAnalytics?.by_sport_and_market && Object.keys(activeAnalytics.by_sport_and_market).length > 0 && (
              <div className="border border-border bg-panel2 overflow-hidden col-span-full">
                <div className="px-2 py-1 text-[10px] text-muted uppercase tracking-wider bg-bg border-b border-border flex justify-between">
                  ...
                  <span className={activeAnalytics.bucket_confidence_enabled ? 'text-success' : 'text-muted2'}>
                    multiplier {activeAnalytics.bucket_confidence_enabled ? 'LIVE' : 'preview (set BUCKET_CONFIDENCE_ENABLED=1)'}
                  </span>
                </div>
                <table className="w-full text-[11px] font-mono">
                  <thead className="bg-bg/50">
                  ...
                  <tbody>
                    {Object.entries(activeAnalytics.by_sport_and_market)
                      .filter(([, v]) => v != null)
                      .sort(([, a], [, b]) => (b?.n ?? 0) - (a?.n ?? 0))
                      .map(([key, v]) => {
                        if (!v) return null;
